...
token *lex_keyword_or_identifier() {
    ...
    if (contains(keywords,name)) {
        return make_token(*(int *)lookup(&keywords,name),0);
    }
	
    return make_token(TOKEN_IDENTIFIER,duplicate_string(name));
}
...
token *lex_numeric_constant() {
    return make_token(TOKEN_CONSTANT,get_number());
}
...
token *lex_operator() {
    ...
    if (p!=NULL) {
        ...
        next_char();
        return make_token(token_table[p-op_chars],0);
    }
	
    if (current_char=='<') {
        if (next_char()=='=') {
            next_char();
            return make_token(TOKEN_LESSEQ,0);
        }
        return make_token(TOKEN_LESS,0);
    }
    if (current_char=='>') {
        if (next_char()=='=') {
            next_char();
            return make_token(TOKEN_GREATEREQ,0);
        }
        return make_token(TOKEN_GREATER,0);
    }
...
list *parse_name() {
    if (current_token->type==TOKEN_IDENTIFIER) {
        list *r = make_var((char *)current_token->value);
        current_token = lex();
...
list *parse_atomic() {
    ...
    switch (current_token->type) {
		case TOKEN_CONSTANT:
			r = make_int(current_token->value);
			current_token = lex();
			return r;
		case TOKEN_IDENTIFIER:
			r = make_var((char *)current_token->value);
			current_token = lex();
			return r;
		case TOKEN_LPAREN:
			current_token = lex();
